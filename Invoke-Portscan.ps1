function Invoke-Portscan
{
    ...
    PROCESS {
        ...
        $version = .13
        $hostList = New-Object -TypeName System.Collections.ArrayList
        $portList = New-Object -TypeName System.Collections.ArrayList
        $hostPortList = New-Object -TypeName System.Collections.ArrayList
        ...
        function Parse-ILHosts
        {
            ...
            Get-Content $HostFile | ForEach-Object -Process {
                Parse-Hosts $_
            }
        }
        ...
        function Parse-IpPorts
        {
            ...
            Get-Content $PortFile | ForEach-Object -Process {
        ...
        try
        {
            ...
            if($Ports)
            {
                $null = Parse-Ports -Ports $Ports -pList $portList
            }
            if($PortFile)
            {
                $null = Parse-IpPorts($PortFile)
            }
            if($portList.Count -eq 0)
            {
                if ($TopPorts)
                {
                    $null = Get-TopPort($TopPorts)
                }
                else
                {
            ...
            if ($ExcludedPorts)
            {
                $null = Remove-Ports -ExcludedPorts $ExcludedPorts
            }
            ...
            if($AllformatsOut)
            {
                ...
                {
                    Write-Warning -Message 'Both -oA specified with other output... going to ignore -oG/-oN/-oX'
                }
            ...
            $null = Parse-Ports -Ports $DiscoveryPorts -pList $hostPortList
            ...
            $portScanCode = {
                ...
                Process
                {
                    $openPorts = New-Object -TypeName System.Collections.ArrayList
                    $closedPorts = New-Object -TypeName System.Collections.ArrayList
                    $filteredPorts = New-Object -TypeName System.Collections.ArrayList

                    $sockets = @{}
                    $timeouts = New-Object -TypeName Hashtable

                    #set maximum $async threads
                    $fThreads = New-Object -TypeName int
                    $aThreads = New-Object -TypeName int
                    $null = [System.Threading.ThreadPool]::GetMaxThreads([ref]$fThreads, [ref]$aThreads)
                    $null = [System.Threading.ThreadPool]::SetMaxThreads($fThreads,$maxthreads)
...
"@
                        ...
                        $null = Register-ObjectEvent -InputObject $bridge -EventName CallbackComplete -Action $Callback
                    ...
                    function Test-Port 
                    {
                        ...
                        try 
                        {
                            ...
                            $sockets[$p] = New-Object -TypeName System.Net.Sockets.TcpClient -ArgumentList $pAddress.AddressFamily
                        }
                        catch 
                        {
                            #we're assuming this is a host name
                            $sockets[$p] = New-Object -TypeName System.Net.Sockets.TcpClient
                        }
...
"@
                        ...
                        $timeouts[$p] = New-Object -TypeName System.Timers.Timer
                        $null = Register-ObjectEvent -InputObject $timeouts[$p] -EventName Elapsed -Action $timeoutCallback
                    ...
                    function PortScan-Alive
                    {
                        ...
                        Catch
                        {
                            Write-Error -Message "Exception trying to host scan $h"
                    ...
                    function Portscan-Port
                    {
                        ...
                        foreach($Port in $portList)
                        {
                            ...
                            Catch
                            {
                                Write-Error -Message "Exception trying to scan $h port $Port"
                                Write-Error -Message $_.Exception.Message
                            }
                        }
                    }
            ...
            while (($saveIteration * $SyncFreq) -lt $hostList.Count)
            {
                ...
                foreach ($iHost in $hostList[$sIndex..$eIndex])
                {
                    ...
                    $null = Start-Job -ScriptBlock $portScanCode -Name $iHost -ArgumentList @($iHost, $SkipDiscovery, $PingOnly, $Timeout, $portList, $hostPortList, $Threads)
                }
